...
```
Note that the specified type must either be declared or `use`d in the scope surrounding the `kosame::table!` call or be an "absolute" path (e.g. `crate::MyType` or `::std::string::String`).
...
Kosame supports both named and anonymous queries. Anonymous queries are defined inline and act as a Rust expression that can be executed immediately. They also allow closuring variables from the surrounding scope as bind parameters for the query:
